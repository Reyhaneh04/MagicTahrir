	...
	switch ($_GET['action']){
		case 'EDIT':
			...
			if (mysqli_query($link,$query) === true){
				...
				unlink ("image/products/".$pro_image);
			}else
...
if ($imagefiletype != "jpg" && $imagefiletype!="png" && $imagefiletype!="jpeg" && $imagefiletype!="gif"){
	echo "فقط پسوند های GIF & PNG,JPEG,JPG برای پرونده مجاز هستند <br/>";
	$uploadok = 0;
}
...
if ($uploadok==0){
	...
	if (move_uploaded_file($_FILES["pro_image"]["tmp_name"],$target_file)){
		echo "پرونده" .$_FILES["pro_image"]["tmp_name"]."با موفقیت به سرویس دهنده میزبان انتقال یافت <br/>";
	}else{
